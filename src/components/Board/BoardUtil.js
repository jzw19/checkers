...
export const showDiagonalMove = (rowAndColumn, rowAndColumnDirection, allyValue, enemyValue, nextAvailableMoves, board) => {
...
export const showMovesUp = (row, column, isWhiteMove, nextAvailableMoves, board) => {
  const allyValue = isWhiteMove ? 1 : 2;
  const enemyValue = isWhiteMove ? 2 : 1;

  nextAvailableMoves = showDiagonalMove([row, column], [-1, -1], allyValue, enemyValue, nextAvailableMoves, board);
  nextAvailableMoves = showDiagonalMove([row, column], [-1, 1], allyValue, enemyValue, nextAvailableMoves, board);

  return nextAvailableMoves;
}

export const showMovesDown = (row, column, isWhiteMove, nextAvailableMoves, board) => {
  const allyValue = isWhiteMove ? 1 : 2;
  const enemyValue = isWhiteMove ? 2 : 1;
...
export const showKingMoves = (row, column, isWhiteMove, nextAvailableMoves, board) => {
  nextAvailableMoves = showMovesUp(row, column, isWhiteMove, nextAvailableMoves, board);
  nextAvailableMoves = showMovesDown(row, column, isWhiteMove, nextAvailableMoves, board);
